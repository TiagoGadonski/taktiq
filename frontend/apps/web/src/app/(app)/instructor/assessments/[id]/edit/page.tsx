...
export default function EditAssessmentPage() {
  ...
  const fetchAssessment = async () => {
    try {
      ...
      // Set postural fields
      setForwardHead((data.forwardHead as SeverityLevel) || 'None');
      setRoundedShoulders((data.roundedShoulders as SeverityLevel) || 'None');
      setAnteriorPelvicTilt((data.anteriorPelvicTilt as SeverityLevel) || 'None');
      setPosteriorPelvicTilt((data.posteriorPelvicTilt as SeverityLevel) || 'None');
      setKneeValgus((data.kneeValgus as SeverityLevel) || 'None');
      setKneeVarus((data.kneeVarus as SeverityLevel) || 'None');
      setFlatFeet((data.flatFeet as SeverityLevel) || 'None');
      setScoliosis((data.scoliosis as SeverityLevel) || 'None');

      // Set physical fields
      setBodyFatPercentage(data.bodyFatPercentage?.toString() || '');
      setMuscleMass(data.muscleMass?.toString() || '');
      setFlexibilityScore(data.flexibilityScore?.toString() || '');
      setStrengthScore(data.strengthScore?.toString() || '');
      setCardioScore(data.cardioScore?.toString() || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await apiClient.put(`/assessments/${assessmentId}`, payload);
      ...
      router.push(`/instructor/assessments/${assessmentId}`);
    } catch (error: any) {
      toast({
        title: 'Erro',
        description: error.response?.data?.message || 'Não foi possível atualizar a avaliação.',
        variant: 'destructive',
      });
    } finally {
    ...
    <div className="container mx-auto p-6 max-w-4xl space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Atualizar informações da avaliação</p>
        </div>
      </div>
